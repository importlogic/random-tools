...
const encrypt = async (message, password, fileName, ext) => {
    ...
    var {data, width, height} = await getPixels(path);
    data = ndarray(new Uint8Array(data), [width, height, 4], [4, 4 * width, 1]);
    ...
    var image = savePixels(data, '.png');
    image.pipe(fs.createWriteStream(`${dir}downloads/${fileName}-converted.png`));
    setTimeout(() => {
